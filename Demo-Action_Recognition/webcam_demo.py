...
import numpy as np
import torch
from mmcv import Config, DictAction, video
...
def main():
    ...
    args = parse_args()
    average_size = args.average_size    # 使用最近N个片段的平均结果作为预测，default=1
    threshold = args.threshold          # 动作识别的分数阈值，只有分数大于阈值的动作类型被显示，default=0
    drawing_fps = args.drawing_fps      # 可视化结果时的最高帧率，default=20
    inference_fps = args.inference_fps  # 进行推理时的最高帧率，default=4
    _video = args.camera_id
    # _video = "test.mp4"

    device = torch.device(args.device)

    cfg = Config.fromfile(args.config)
    cfg.merge_from_dict(args.cfg_options)       # TODO 替换默认值

    model = init_recognizer(cfg, args.checkpoint, device=device)

    camera = cv2.VideoCapture(_video) 

    data = dict(img_shape=None, modality='RGB', label=-1)   # 图像及标签信息
